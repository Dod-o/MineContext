...
<a href="https://github.com/volcengine/MineContext/releases/download/v0.1.8/MineContext-0.1.8.dmg">🖥️ Mac 版下载</a> · <a href="https://github.com/volcengine/MineContext/releases/download/v0.1.8/MineContext-0.1.8-setup.exe">💻 Windows 版下载</a> · <a href="https://github.com/volcengine/MineContext/releases">🐧 Linux / Ubuntu 下载</a>
...
如果使用 4K/8K 显示器时发现截图里的文字被识别得很模糊，可以调高截图送入视觉模型前的最大处理尺寸：

1. 打开后端调试页 `http://localhost:1733`。
2. 进入【设置】->【处理配置】。
3. 在【截图处理】里把【最大图片尺寸】从默认的 `1920` 调高到 `3072` 或 `4096`，保存后重新开始录屏。

也可以直接修改用户配置里的 `processing.screenshot_processor.max_image_size`。数值越大，文字细节越清晰，但 VLM 请求的图片体积、处理时间和 token/API 成本也会增加；如果仍然识别不准，请确认当前视觉模型本身支持高分辨率图片理解。
